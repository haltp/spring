	...
	public void insert(EmpVO vo) {
		try {
			...
			String sql = "INSERT INTO EMPLOYEES (EMPLOYEE_ID, " + " MANAGER_ID, " + " LAST_NAME, " + " EMAIL, " + " HIRE_DATE, "
					+ " JOB_ID, FIRST_NAME, PHONE_NUMBER, DEPARTMENT_ID) " + " VALUES(?,?,?,?,?,?,?,?,?)";
			...
			pstmt.setString(7, vo.getFirst_name());
	...
	public void update(EmpVO vo) {
		try {
			...
			String sql = "UPDATE EMPLOYEES SET FIRST_NAME = ?, LAST_NAME = ?, EMAIL=?, "
					   + "PHONE_NUMBER=?, HIRE_DATE=?, JOB_ID=?, MANAGER_ID=?, DEPARTMENT_ID=? "
					   + "WHERE EMPLOYEE_ID=?";
			// 3. execute(실행)
			PreparedStatement pstmt = conn.prepareStatement(sql);
			...
			pstmt.setDate(5, vo.getHire_date());
			pstmt.setString(6, vo.getJob_id());
			pstmt.setString(7, vo.getManager_id());
			pstmt.setString(8, vo.getDepartment_id());
			pstmt.setString(9, vo.getEmployee_id());
